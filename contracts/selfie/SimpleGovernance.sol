...
contract SimpleGovernance is ISimpleGovernance {
    ...
    function executeAction(uint256 actionId) external payable returns (bytes memory) {
        ...
        (bool success, bytes memory returndata) = actionToExecute.target.call{value: actionToExecute.value}(actionToExecute.data); //@audit should call the emergency exit
        if (!success) {
    ...
    function _canBeExecuted(uint256 actionId) private view returns (bool) {
        ...
        return actionToExecute.executedAt == 0 && timeDelta >= ACTION_DELAY_IN_SECONDS; //@audit should move the time 2 days forward
    }
    ...
    function _hasEnoughVotes(address who) private view returns (bool) {
        ...
        return balance > halfTotalSupply; //@audit should be true.
    }
}
