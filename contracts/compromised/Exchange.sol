...
contract Exchange is ReentrancyGuard {
    ...
    function buyOne() external payable nonReentrant returns (uint256 id) {
        ...
        uint256 price = oracle.getMedianPrice(token.symbol());  //@audit price is fetched from the oracle.
        if (msg.value < price)
            revert InvalidPayment();
    ...
    function sellOne(uint256 id) external nonReentrant {//@audit selling our nfts to the contract.
